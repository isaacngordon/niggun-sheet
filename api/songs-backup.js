...
function parseCSV(csvText) {
    ...
    for (let i = 1; i < lines.length; i++) { // Skip header row
        ...
        if (fields.length >= 4) {
            ...
            // Clean up quoted fields and escaped quotes
            result.push({
                search_title: search_title.replace(/^"|"$/g, ''), // Remove surrounding quotes
                title: title.replace(/^"|"$/g, ''),
                lyrics: lyrics.replace(/^"|"$/g, '').replace(/""/g, '"'), // Remove surrounding quotes and unescape internal quotes
...
async function handler(req, res) {
    ...
    try {
        ...
        if (cachedSongs && (now - cacheTimestamp) < CACHE_DURATION) {
            console.log(`Returning cached songs (${cachedSongs.length} songs)`);
        ...
        try {
            ...
            console.log(`Loaded ${songs.length} songs from Google Sheets API`);
